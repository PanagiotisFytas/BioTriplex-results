...
for mode in ["test", "val", "train"]:
    suffix = f"mode_{mode}_sglang.json"
    if system == "hpc":
    ...
    for output_file in os.listdir(input_dir):
        ...
        precision = {entity_type: TP[entity_type] / (TP[entity_type] + FP[entity_type]) for entity_type in ["genes", "diseases", "relations"]}
        recall = {entity_type: TP[entity_type] / (TP[entity_type] + FN[entity_type]) for entity_type in ["genes", "diseases", "relations"]}
        f1 = {"entity_type": 0 for entity_type in ["genes", "diseases", "relations"]}
        for entity_type in ["genes", "diseases", "relations"]:
            if precision[entity_type] + recall[entity_type] == 0:
                f1[entity_type] = 0
            else:
                f1[entity_type] = 2 * precision[entity_type] * recall[entity_type] / (precision[entity_type] + recall[entity_type])
        print("Exact match")
        ...
        partial_precision = {entity_type: TP[entity_type] / (TP[entity_type] + FP[entity_type]) for entity_type in ["genes", "diseases", "relations"]}
        partial_recall = {entity_type: TP[entity_type] / (TP[entity_type] + FN[entity_type]) for entity_type in ["genes", "diseases", "relations"]}
        ...
        for entity_type in ["genes", "diseases", "relations"]:
            if partial_precision[entity_type] + partial_recall[entity_type] == 0:
                ...
                partial_f1[entity_type] = 2 * partial_precision[entity_type] * partial_recall[entity_type] / (partial_precision[entity_type] + partial_recall[entity_type])
        print("Partial match")
        ...
        if overall_partial_precision + overall_partial_recall == 0:
            ...
            overall_partial_f1 = 2 * overall_partial_precision * overall_partial_recall / (overall_partial_precision + overall_partial_recall)
        print("\tOverall")
        ...
        with open(f"{output_dir}{prefix}mode_{mode}_results_sglang.txt", "w") as f:
            f.write("Exact match\n")
