...
def output_to_relation(output):
    if "a" in output:
        ...
        return "pathological role"
    if "b" in output:
        if output != "b)":
            warnings.warn(f"Output {output} is not in the expected format")
        return "causative activation"
    if "c" in output:
        if output != "c)":
            warnings.warn(f"Output {output} is not in the expected format")
        return "causative inhibition"
    if "d" in output:
        if output != "d)":
            warnings.warn(f"Output {output} is not in the expected format")
        return "causative mutation"
    if "e" in output:
        if output != "e)":
            warnings.warn(f"Output {output} is not in the expected format")
        return "modulator decrease disease"
    if "f" in output:
        if output != "f)":
            warnings.warn(f"Output {output} is not in the expected format")
        return "modulator increase disease"
    if "g" in output:
        if output != "g)":
            warnings.warn(f"Output {output} is not in the expected format")
        return "biomarker"
    ...
    if "j" in output:
        if output != "j)":
            warnings.warn(f"Output {output} is not in the expected format")
        return "increased expression"
    if "k" in output:
        if output != "k)":
            warnings.warn(f"Output {output} is not in the expected format")
        return "decreased expression"
    if "l" in output:
        if output != "l)":
            warnings.warn(f"Output {output} is not in the expected format")
        return "epigenetic marker"
    if "m" in output:
        if output != "m)":
            warnings.warn(f"Output {output} is not in the expected format")
        return "therapy resistance"
    if "n" in output:
        if output != "n)":
            warnings.warn(f"Output {output} is not in the expected format")
    ...
    if "o" in output:
        if output != "o)":
            warnings.warn(f"Output {output} is not in the expected format")
        return "negative prognostic marker"
    if "p" in output:
        if output != "p)":
            warnings.warn(f"Output {output} is not in the expected format")
        return "positive prognostic marker"
    if "q" in output:
        if output != "q)":
            warnings.warn(f"Output {output} is not in the expected format")
        return "therapeutic target"
    if "r" in output:
        if output != "r)":
            warnings.warn(f"Output {output} is not in the expected format")
...
for mode in ["test", ]:#"val", "train"]:
    ...
    if system == "hpc":
        gold_file = f"/home/pf376/biotriplex/data/{mode}_gold_qa.txt"
    else:
        gold_file = f"/mnt/nas_home/pf376/Documents/biotriplex/data/{mode}_gold_qa.txt"
    print("#######################################################")
    print("#######################################################")
    print(f"################# Mode: {mode} #######################")
    print("#######################################################")
    ...
    for output_file in os.listdir(input_dir):
        if not output_file.endswith(suffix):
            continue
        if "biotriplex_qa" not in output_file and (mode not in output_file):
            continue
        print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
        print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
        ...
        with open(input_dir + "/" + output_file, "r") as f:
            ...
            outputs = {item["doc_key"]: item["output"] for item in [json.loads(line) for line in f.readlines()]}
        ...
        c = classification_report(y_true, y_pred, target_names=relations, digits=4, labels=range(len(relations)))
        ...
        print(pd.DataFrame(confusion_matrix(y_true, y_pred, labels=range(len(relations))), index=relations, columns=relations).to_csv(sep="\t"))
