...
def output_to_relation(outputs):
    ...
    for output in outputs:
        if "a" in output:
            ...
            yield "disease causing"
        elif "b" in output:
            if output != "b)":
                warnings.warn(f"Output {output} is not in the expected format")
            yield "disease modulating"
        elif "c" in output:
            if output != "c)":
                warnings.warn(f"Output {output} is not in the expected format")
...
for mode in ["test", ]:#"val", "train"]:
    ...
    if system == "hpc":
        gold_file = f"/home/pf376/biotriplex/data/{mode}_gold_general_grouped_qa.txt"
    else:
        gold_file = f"/mnt/nas_home/pf376/Documents/biotriplex/data/{mode}_gold_general_grouped_qa.txt"
    print("#######################################################")
    print("#######################################################")
    print(f"################# Mode: {mode} #######################")
    print("#######################################################")
    ...
    for output_file in os.listdir(input_dir):
        if not output_file.endswith(suffix):
            continue
        if ("biotriplex" not in output_file) or ("qamc" not in output_file) or (mode not in output_file) or\
                ("biotriplex_qa" not in output_file) or ("1shot" in output_file) or ("2shot" in output_file) or\
                ("3shot" in output_file) or ("4shot" in output_file) or ("genrel" not in output_file) or \
                ("genrel2" not in output_file):
            continue
        print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
        print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
        ...
        for doc_key, output in outputs.items():
            ...
            try:
                if "qamc" in output_file and "no_peft" not in output_file:
                    output = output.split("### Response:\n")[-1].strip()
                elif "0shot" or "shot" in output_file:
                    # print("1: ", output)
                    output = output.split("assistant")[-1].strip()
                    print("output1: ", output)
                    if "### Response:\n" in output:
                        output = output.split("### Response:\n")[-1].strip()
                    # if there are not parentheses, make a "," into a ",)"
                    if ")" not in output:
                        output = output.replace(",", "),")
                    # add a ")" to the end of the string if it is not there
                    if output[-1] != ")" and output[-1] in ["a", "b", "c", "d", "e", "f", "g", "h", "i", "j", "k",
                                                             "l", "m", "n", "o", "p", "q", "r", "s", "t", "u"]:
                        output += ")"
