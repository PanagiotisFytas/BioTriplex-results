...
for mode in ["test", "val", "train"]:
    ...
    if system == "hpc":
        gold_file = f"/home/pf376/biotriplex/data/{mode}_gold_general_qa.txt"
    else:
        gold_file = f"/mnt/nas_home/pf376/Documents/biotriplex/data/{mode}_gold_general_qa.txt"
    print("#######################################################")
    print("#######################################################")
    print(f"################# Mode: {mode} #######################")
    print("#######################################################")
    ...
    for output_file in os.listdir(input_dir):
        if not output_file.endswith(suffix):
            continue
        if "_biotriplex_genrel_qa_" not in output_file:
            continue
        print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
        print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
        ...
        per_class_f1 = {
            ...
            for relation in relations}
        per_class_acc = {relation: true_predictions[relation] / total_samples[relation] for relation in relations}
        ...
        c = classification_report(y_true, y_pred, target_names=relations, digits=4)
        ...
        print(pd.DataFrame(confusion_matrix(y_true, y_pred), index=relations, columns=relations).to_csv(sep="\t"))
